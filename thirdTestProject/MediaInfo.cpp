...
MediaInfo::operator Tags() const
{
	...
	const std::string gainTrack = GainToString(m_GainAlbum);
	if (!gainTrack.empty())
	{
		tags.insert(Tags::value_type(Tag::GainAlbum, gainTrack));
	}
	return tags;
}
...
void MediaInfo::SetBitsPerSample(long bitsPerSample)
{
...
void MediaInfo::SetTitle(const std::wstring& genre)
{
	m_Genre = genre;
}
...
std::wstring MediaInfo::GetArtworkID(const bool checkFolder) const
{
	...
	if (checkFolder && artworkID.empty() && !GetFilename().empty() && (Source::File == GetSource()))
	{
		...
		for (auto artworkFileName = artworkFileTypes.begin(); artworkID.empty() && (artworkFileTypes.end() != artworkFileType); artworkFileType++)
		{
...
{
	const bool isDuplicate = std::tie(m_Filesize, m_Duration, m_SampleRate, m_BitsPerSample, m_Channels, m_Artist, m_Title, m_Album, m_Genre, m_Year,
		m_Comment, m_Track, m_Version, m_GainTrack, m_GainAlbum, m_ArtworkID) == std::tie(o.m_Filesize, o.m_Duration, o.m_SampleRate, o.m_BitsPerSample,
		o.m_Channels, o.m_Artist, o.m_Title, o.m_Album, o.m_Genre, o.m_Year,o.m_Comment, o.m_Track, o.m_Version, o.m_GainTrack, o.m_GainAlbum, o.m_ArtworkID);
	return isDuplicate;
}
...
bool MediaInfo::GetCommonInfo(const List& mediaList, MediaInfo& commonInfo)
{
	...
	{
		commonInfo.SetGenre(*albums.begin());
		hasCommonInfo = true;
	}
