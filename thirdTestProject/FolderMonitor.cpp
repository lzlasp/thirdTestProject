...
DWORD WINAPI FolderMonitor::MonitorThreadProc(LPVOID lpParam)
{
	...
	if (nullptr != monitorInfo)
	{
		...
		DWORD bytesReturns = 0;

		OVERLAPPED overlapped = {};
		overlapped.hEvent = CreateEvent(nullptr/*securityAttributes*/, TRUE/*manualReset*/, FALSE/*initialState*/, L""/*lpName*/);
		if (nullptr != overlapped.hEvent)
		{
			...
			while (!cancelled)
			{
				if (ReadDirectoryChangesW(monitorInfo->DirectoryHandle, &buffer[0], bufferSize, watchSubtree, notifyFilter, &bytesReturns, &overlapped, nullptr))
				{
					if (WAIT_OBJECT_0 == WaitForMultipleObjects(2/*count*/, waitHandles, FALSE/*waitAll*/, INFINITE))
					{
						if (GetOverlappedResult(monitorInfo->DirectoryHandle, &overlapped, &bytesReturns, FALSE/*WAIT*/))
						{
									...
									case FILE_ACTION_ADDED:
									{
										...
										if ((INVALID_FILE_ATTRIBUTES != attributes) && !(FILE_ATTRIBUTE_HIDDEN & attributes) && !(FILE_ATTRIBUTE_SYSTEM & attributes))
										{
											if (ChangeType::FileChange == monitorInfo->ChangeType)
											{
												...
												std::lock_guard<std::mutex> lock(monitorInfo->PendingMutext);
										...
										if (0 != notifyInfo->NextEntryOffset)
										{
											...
											if (FILE_ACTION_RENAMED_NEW_NAME == notifyInfo->Action)
											{
												...
												if ((INVALID_FILE_ATTRIBUTES != attributes) && !(FILE_ATTRIBUTE_HIDDEN & attributes) && !(FILE_ATTRIBUTE_SYSTEM & attributes))
												{
													...
													monitorInfo->Callback((ChangeType::FileChange == monitorInfo->ChangeType) ? FolderMonitor::Event::FileRename : FolderMonitor::Event::FolderRenamed, filename, filename);
			...
			GetOverlappedResult(monitorInfo->DirectoryHandle, &overlapped, &bytesReturns, TRUE/*wait*/);
...
DWORD WINAPI FolderMonitor::AddFileThreadProc(LPVOID lpParam)
{
	...
	if (nullptr != monitorInfo)
	{
		...
		const long long minimumPendingTime = 1ll * 10000000ll/*100-anaosecond intervals*/;
		...
		while (WAIT_OBJECT_0 != WaitForSingleObject(monitorInfo->CancelHandle, retryInterval))
		{
			...
			while (monitorInfo->PendingMap.end() != fileIter)
			{
				...
				{
					const DWORD desireAccess = GENERIC_READ;
					const DWORD shareMode = FILE_SHARE_READ | FILE_SHARE_WRITE | FILE_SHARE_DELETE;
					const DWORD creationDisposition = OPEN_EXISTING;
					const DWORD flags = 0;
					const HANDLE fileHandle = CreateFile(fileName.c_str(), desireAccess, shareMode, nullptr/*securityAttributes*/, creationDisposition, flags, nullptr/*template*/);
...
FolderMonitor::FolderMonitor(const HWND hwnd) : m_hWnd(hwnd),m_Monitors()
{
...
bool FolderMonitor::AddMonitor(const std::wstring folder, const EventCallback callback, const ChangeType changeType)
{
	...
	if (nullptr != callback)
	{
		...
		monitorInfo->DirectoryHandle = CreateFile(monitorInfo->DirectoryPath.c_str(), desireAccess, shareMode, nullptr/*securityAttrbiutes*/, creationDisposition, flags, nullptr/*template*/);
		if (INVALID_HANDLE_VALUE != monitorInfo->DirectoryHandle)
		{
			...
			monitorInfo->CancelHandle = CreateEvent(nullptr/*securityAttributes*/, TRUE/*manualReset*/, FALSE/*initialSate*/, L""/*name*/);
			if (nullptr != monitorInfo->CancelHandle)
			{
				monitorInfo->MonitorThreadHandles = CreateThread(nullptr/*attributes*/, 0/*stackSize*/, &MonitorThreadProc, reinterpret_cast<LPVOID>(monitorInfo)/*param*/, 0/*flags*/, nullptr/*threadId*/);
				...
				if ((nullptr != monitorInfo->MonitorThreadHandles) && (nullptr != monitorInfo->AddFileThreadHandle))
				{
					auto monitorIter = m_Monitors.insert(FolderMap::value_type(folder, Monitors())).first;
					if (m_Monitors.end() != monitorIter)
					{
						monitorIter->second.push_back(monitorInfo);
						success = true;
					}
				}
			}
...
void FolderMonitor::CloseMonitor(MonitorInfo* monitor)
{
	...
	if (nullptr != monitor->MonitorThreadHandles)
	{
		WaitForSingleObject(monitor->MonitorThreadHandles, INFINITE);
		CloseHandle(monitor->MonitorThreadHandles);
	}
